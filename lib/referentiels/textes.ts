/**
 * Référentiels du SIRH du METP.
 *
 * Écrit d'après le cahier fonctionnel de la DGARH. Chaque entité porte sa
 * provenance : « TEXTE » = corroboré par une source publique, « A_VERIFIER »
 * = hypothèse à confirmer sur le Journal officiel, « RECOMMANDATION » = choix
 * de conception. Cf. cahier §01 — ne jamais présenter du A_VERIFIER comme du droit.
 */

import type { Provenance } from "@/lib/types";

export const APP_NAME = "NEXUS-METP";
export const APP_TAGLINE = "Système Intégré de Gestion des Ressources Humaines";
export const MINISTERE_NOM = "Ministère de l'Enseignement Technique et Professionnel";
/**
 * Marque de l'État. Les deux fichiers vivent dans /public : pour poser les
 * originaux du ministère, on les écrase, et toute l'application suit — écran
 * d'ouverture, connexion, barre latérale, en-têtes de documents.
 *
 * Ils sont servis par l'application elle-même, et non depuis un hébergeur
 * tiers : les armoiries d'un ministère ne doivent pas dépendre d'un domaine
 * que la DGARH ne contrôle pas, et la plateforme doit s'afficher entièrement
 * hors ligne.
 */
/**
 * Le blason, cherché dans cet ordre. On dépose le fichier officiel dans
 * /public sous l'un de ces noms — TEL QUEL, sans le renommer ni le convertir —
 * et l'application le sert partout : écran d'ouverture, connexion, barre
 * latérale, filigrane, en-têtes de documents imprimés.
 *
 * Le tracé vectoriel de dépannage est volontairement le DERNIER : il ne sert
 * que tant que l'original n'est pas là, et s'efface dès qu'il l'est.
 */
/**
 * Les fichiers de marque, résolus à la construction contre /public par
 * `scripts/resoudre-marque.mjs`. Déposez un fichier officiel dans /public et
 * reconstruisez : il prend sa place, sans toucher au code. Les résoudre
 * d'avance évite d'essayer à l'exécution des noms qui n'existent pas —
 * c'étaient six requêtes perdues par ouverture de page, et un journal
 * serveur qui donnait à croire à une panne.
 */
export {
  ARMOIRIES_SOURCES, LOGO_SOURCES, DRAPEAU_URL, FOND_MINISTERE,
} from "./marque";
import { ARMOIRIES_SOURCES } from "./marque";

/** @deprecated Utilisez ARMOIRIES_SOURCES, ou le composant <Armoiries />. */
export const ARMOIRIES_URL = ARMOIRIES_SOURCES[ARMOIRIES_SOURCES.length - 1];
/** @deprecated Idem. */
export const LOGO_URL = ARMOIRIES_URL;

/* ------------------------------------------------------------------ */
/* Textes de référence — cahier §18                                    */
/* ------------------------------------------------------------------ */

export const TEXTES = {
  /** Statut en vigueur. Remplace la loi n° 021-89 du 14 novembre 1989. */
  STATUT: "Loi n° 68-2022 du 16 août 2022 portant statut général de la fonction publique",
  /* Le décret dont procèdent les neuf arrêtés d'organisation : chacun d'eux
     le vise en tête. Son texte n'a pas été consulté — les arrêtés, si. */
  DECRET_ORG: "Décret n° 2022-118 du 22 mars 2022 portant organisation du ministère",
  ARR_25564: "Arrêté n° 25564 du 17 octobre 2022 (JO 2022-44)",
  ARR_25565: "Arrêté n° 25565 du 17 octobre 2022 (JO 2022-44)",
  ARR_25566: "Arrêté n° 25566 du 17 octobre 2022 (JO 2022-44)",
  ARR_25567: "Arrêté n° 25567 du 17 octobre 2022 (JO 2022-44)",
  ARR_25568: "Arrêté n° 25568 du 17 octobre 2022 (JO 2022-44)",
  ARR_25569: "Arrêté n° 25569 du 17 octobre 2022 (JO 2022-45)",
  ARR_25570: "Arrêté n° 25570 du 17 octobre 2022 (JO 2022-45)",
  ARR_25571: "Arrêté n° 25571 du 17 octobre 2022 (JO 2022-45)",
  ARR_25572: "Arrêté n° 25572 du 17 octobre 2022 (JO 2022-45)",
} as const;

export const PROVENANCE_LABELS: Record<Provenance, string> = {
  TEXTE: "Texte",
  A_VERIFIER: "À vérifier",
  RECOMMANDATION: "Recommandation",
};

/**
 * Ce qui manque encore pour figer le référentiel. Affiché dans l'espace DGARH.
 *
 * **Quatre des six lacunes d'hier sont refermées**, et il faut dire par quoi :
 * les Journaux officiels n° 44-2022 et 45-2022 portent une couche de texte,
 * les neuf arrêtés d'organisation du 17 octobre 2022 ont été dépouillés, et
 * l'organigramme est rebâti sur eux. Le cabinet n'est plus « le schéma
 * constant des ministères congolais » mais l'arrêté n° 25564 ; les services et
 * bureaux de la DGARH ne sont plus « plausibles » mais l'arrêté n° 25567 ; les
 * relais départementaux de contrôle portent enfin le nom du texte — antennes
 * départementales **d'encadrement** — et le décret dont procèdent les arrêtés
 * est identifié par leurs visas.
 *
 * Ce qui reste ci-dessous reste pour une raison : **le texte n'existe pas
 * dans les sources ouvertes**, et non qu'on ne l'a pas ouvert. La différence
 * est la seule qui compte quand on décide de mettre en service.
 */
export const LACUNES = [
  {
    sujet: "Liste réelle des établissements",
    manque: "Carte scolaire de l'enseignement technique et professionnel",
    ou: "Document non publié — les établissements de l'outil sont un gabarit",
    consequence:
      "C'est la lacune la plus lourde : quarante-sept lycées et collèges portés par "
      + "l'outil sont un gabarit réparti sur les douze départements, et non la carte "
      + "réelle. Les effectifs de la vue nationale suivent donc une répartition "
      + "vraisemblable. Aucun arrêté d'organisation ne la donne : elle se demande à la "
      + "direction générale de l'enseignement technique.",
  },
  {
    sujet: "Découpage des inspections interdépartementales",
    manque: "Liste des inspections interdépartementales et des départements de chacune",
    ou: "Arrêté n° 25570 lu : il fixe leurs attributions et leur organisation, pas leur nombre ni leur ressort",
    consequence:
      "L'arrêté décrit ce qu'est une inspection interdépartementale et ce qu'elle "
      + "contient — l'outil le reprend mot pour mot. Le regroupement en quatre "
      + "inspections (Sud, Centre, Cuvettes, Nord) est en revanche une reconstitution, "
      + "et c'est pourquoi ces quatre entités-là portent « à vérifier » quand leurs "
      + "organes internes portent « texte ».",
  },
  {
    sujet: "Décret d'attributions et d'organisation du ministère",
    manque: "Texte intégral du décret n° 2022-118 du 22 mars 2022",
    ou: "Référence établie par les visas des neuf arrêtés ; dispositif non consulté",
    consequence:
      "Le chaînon est nommé : chaque arrêté d'octobre 2022 le vise en tête, ce qui "
      + "confirme que le ministère compte bien quatre directions générales, une "
      + "inspection générale et un cabinet. Ce que son dispositif ajouterait, ce sont "
      + "les attributions de l'échelon ministériel lui-même.",
  },
  {
    sujet: "Grille indiciaire et statuts particuliers",
    manque: "Textes d'application de la loi n° 68-2022",
    ou: "Loi identifiée et publiée ; grille et statuts particuliers non consultés",
    consequence:
      "Les catégories I à III et leurs trois échelles sont établies par la loi. "
      + "Les indices chiffrés portés par l'outil restent, eux, une reconstitution — "
      + "et la valeur du point indiciaire n'est pas renseignée, faute d'être connue.",
  },
  {
    sujet: "Contradiction interne de l'arrêté n° 25568",
    manque: "Rectificatif au Journal officiel, ou lecture de l'original signé",
    ou: "Arrêté n° 25568 (JO 2022-44) — article 2 contre chapitre 3",
    consequence:
      "L'article 2 énumère une « direction des finances et du matériel » ; le chapitre 3 "
      + "et les articles 21 et 22 décrivent une « direction de l'administration, des "
      + "finances et du matériel ». L'outil retient la seconde, qui est celle que le "
      + "texte développe. La divergence est signalée plutôt que tranchée en silence.",
  },
];
/* ------------------------------------------------------------------ */
